...
def load_ea_data(ea_dir: Path):
    ...
    if not geojson_files:
        ...
        zip_file = ea_dir / "Flood_Map_for_Planning_Flood_Zones_plus_Climate_Change.geojson.zip"
        if zip_file.exists():
...
def compact_dissolved(dissolved, precision_grid=0.00001):
    ...
    try:
        ...
        pass
    return make_geometries_valid(dissolved)
...
def write_summary(gdf_detailed, gdf_dissolved, docs_dir, bounds_wgs84, tolerance):
    ...
    summary_file = docs_dir / "ea-flood-zones-cc-summary.json"
...
def main():
    ...
    parser.add_argument(
        "--input-dir",
        type=Path,
        default=Path("/Volumes/DevProjects/data/leicester-flood-map/ea-flood-zones-cc"),
    ...
    print(f"📊 Normalized columns: {gdf.columns.tolist()}")
    ...
    detailed_file = docs_dir / "ea-flood-zones-cc-detailed.geojson"
    gdf_clip.to_file(
        detailed_file,
        driver="GeoJSON",
    ...
    dissolved_input = gdf_clip[["flood_zone", "geometry"]].copy()
    dissolved = dissolved_input.dissolve(by="flood_zone", as_index=False)
    dissolved = compact_dissolved(dissolved, precision_grid=precision_grid)

    dissolved_file = docs_dir / "ea-flood-zones-cc.geojson"
    dissolved.to_file(
        dissolved_file,
        driver="GeoJSON",
        coordinate_precision=geojson_precision,
